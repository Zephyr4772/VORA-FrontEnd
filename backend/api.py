...
def build_llm_prompt(query: str, results: dict) -> tuple[str, list]:
    ...
    # Cap total context to ~15K chars to avoid Gemini quota burnout
...
@app.get("/api/health")
def health_check():
    return {"status": "healthy", "version": "1.0", "index": "chromadb"}

@app.post("/api/start-ollama")
def start_ollama():
    try:
        # On Windows, this will start the Ollama background process silently
        import subprocess
        import os
        subprocess.Popen(
            ["ollama", "serve"],
...
def debug_retrieve(q: str = "right to privacy Article 21", n: int = 3):
    """
    Test endpoint — returns raw ChromaDB results without LLM.
    Use to confirm RAG is actually pulling real case content.
    e.g. GET /api/debug/retrieve?q=habeas+corpus&n=3
    """
    try:
        col = get_collection()
        results = col.query(query_texts=[q], n_results=n)
        docs = results["documents"][0]
        metas = results["metadatas"][0]
        return {
            "query": q,
            ...
            "cases": [
                {
                    ...
                    "bench": m.get("bench", "?")[:80],
                    "excerpt_preview": d[:300]
                }
...
@app.get("/api/models")
def get_models():
    """Fetch available Ollama chat models (excludes embedding-only models)."""
    EMBEDDING_MODELS = {"nomic-embed-text", "mxbai-embed-large", "all-minilm", "nomic-embed"}
    try:
        import requests
        ...
        if r.status_code == 200:
            ...
            chat_models = [
                ...
                if not any(em in m["name"].lower() for em in EMBEDDING_MODELS)
            ]
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if os.path.exists(DRIVE_MAP_FILE):
                with open(DRIVE_MAP_FILE, 'r', encoding='utf-8') as f:
                    drive_id_map = json.load(f)
            return {"status": "success", "message": "Drive sync completed", "output": result.stdout}
        else:
            return {"status": "error", "message": "Drive sync failed", "output": result.stderr}
    except Exception as e:
        return {"status": "error", "message": str(e)}

# ── Serve React Frontend ─────────────────────────────────────────────────────
# Mount built Vite assets — API routes defined above always take priority
_DIST = os.path.join(os.path.dirname(__file__), "webpage", "dist")
if os.path.isdir(_DIST):
    app.mount("/assets", StaticFiles(directory=os.path.join(_DIST, "assets")), name="assets")

    @app.get("/{full_path:path}", include_in_schema=False)
    def spa_fallback(full_path: str):
        """Serve static files from dist; fall back to index.html for SPA routes."""
    ...
    print("[WARN] webpage/dist not found — run 'npm run build' inside webpage/ to serve the frontend.")

...
const PromptInputTextarea: React.FC<PromptInputTextareaProps & React.ComponentProps<typeof Textarea>> = ({
  ...
  return (
    <Textarea
      ...
      className={cn("text-base w-full p-2 h-auto max-h-[160px] overflow-y-auto no-scrollbar outline-none focus:outline-none resize-none", className)}
      disabled={disabled}
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  const { onSend = () => {}, isLoading = false, placeholder = "Ask anything...", className, inputValue, onInputChange } = props;
  
  // Use controlled input if provided, otherwise fallback to local state
  ...
  return (
    <>
      <PromptInput
        ...
        className={cn(
          "w-full bg-[#111318] border border-[rgba(255,255,255,0.08)] shadow-2xl transition-all duration-300 ease-in-out",
      ...
      >
        {files.length > 0 && !isRecording && (
          <div className="flex flex-wrap gap-2 p-0 pb-1 transition-all duration-300">
            {files.map((file, index) => (
              <div key={index} className="relative group pl-2 pt-2">
        ...
        >
          <PromptInputTextarea
            ...
            className="text-base text-white placeholder-gray-500"
          />
        </div>

...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="text-[10px] font-bold uppercase tracking-[1.4px] mb-2 font-tags" style={{ color: '#888888' }}>{children}</p>;
}

function Divider() {
  return <div style={{ height: '1px', background: 'rgba(209,111,84,0.1)' }} />;
}
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <div className="relative">
        <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: '#888888' }} />
        <input
          value={q}
          ...
          style={{
            ...
            border: '1px solid rgba(209,111,84,0.2)', borderRadius: 'var(--radius-sm)',
            padding: '8px 10px 8px 30px', fontSize: '13px',
            color: '#333333', outline: 'none',
          }}
          onFocus={e => { (e.target as HTMLInputElement).style.borderColor = '#888888'; setOpen(true); }}
          onBlur={e => (e.target as HTMLInputElement).style.borderColor = 'rgba(209,111,84,0.2)'}
        />
        {q && (
          <button onClick={() => { setQ(''); onChange(''); }} className="absolute right-2 top-1/2 -translate-y-1/2 p-1" style={{ color: '#888888' }}>
            <X size={12} />
          </button>
        )}
      </div>
      {open && filtered.length > 0 && (
        <div
          ...
          style={{ background: '#FFFFFF', border: '1px solid rgba(209,111,84,0.2)', borderRadius: 'var(--radius-sm)' }}
        >
          {filtered.map(opt => (
            <button
              key={opt}
  ...
  return (
    <div ref={ref}>
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {tags.map(t => (
            <span key={t} className="flex items-center gap-1.5 px-2 py-1 text-[10px] uppercase tracking-[1px] font-tags"
              style={{ background: 'transparent', color: '#333333', border: '1px solid rgba(209,111,84,0.2)', borderRadius: 'var(--radius-sm)' }}>
              {t}
      ...
      <div className="relative">
        ...
        {input && (
          <button className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] uppercase tracking-[1px] px-2 py-1 font-tags"
            style={{ background: '#333333', color: '#FFFFFF', borderRadius: 'var(--radius-sm)' }}
      ...
      {open && (filtered.length > 0 || suggestions.length > 0) && (
        <div className="mt-1 overflow-hidden" style={{ border: '1px solid rgba(209,111,84,0.2)', background: '#FFFFFF', borderRadius: 'var(--radius-sm)' }}>
          {(input ? filtered : suggestions.filter(s => !tags.includes(s)).slice(0, 8)).map(s => (
            <button key={s} className="w-full text-left px-3 py-2 text-[12px] transition-colors flex items-center gap-2 font-tags"
              style={{ color: '#888888' }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(128,128,128,0.1)'; (e.currentTarget as HTMLElement).style.color = '#333333'; }}
              ...
              <span className="font-bold uppercase tracking-[1px]" style={{ color: '#333333' }}>{s}</span>
            </button>
          ))}
        </div>
      )}
    ...
    <div>
      ...
      <div className="relative mb-6 px-1" style={{ height: '20px' }}>
        ...
        <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 h-px"
          style={{ background: 'rgba(209,111,84,0.2)' }} />
        {/* Active fill */}
        <div className="absolute top-1/2 -translate-y-1/2 h-px"
          style={{ background: '#333333', left: `${pct(range[0])}%`, right: `${100 - pct(range[1])}%` }} />
        ...
        <div className="absolute top-1/2 -translate-y-1/2 pointer-events-none"
          ...
          <div className="w-3 h-3" style={{ background: '#333333', borderRadius: '50%' }} />
        </div>
        {/* Visual thumb TO */}
        <div className="absolute top-1/2 -translate-y-1/2 pointer-events-none"
        ...
        {([['From', range[0], handleFrom, min, range[1] - 1], ['To', range[1], handleTo, range[0] + 1, max]] as const).map(
          ([label, val, handler, lo, hi]) => (
            <div key={String(label)}>
              <p className="text-[10px] uppercase tracking-[1px] mb-1.5 font-tags" style={{ color: '#888888' }}>{label}</p>
              <input
                ...
                onFocus={e => (e.target as HTMLInputElement).style.borderColor = '#888888'}
                onBlur={e => (e.target as HTMLInputElement).style.borderColor = 'rgba(209,111,84,0.2)'}
              />
            </div>
          )
...
export default function FilterPanel({ show, onClose, filters, setFilters, cases }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/20 backdrop-blur-sm" onClick={onClose}>
      <div
        className="bg-white/95 backdrop-blur-xl text-[#333] w-[460px] h-[85vh] rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-orange-900/10 overflow-hidden flex flex-col transform transition-all animate-in fade-in zoom-in-95 duration-200"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 shrink-0" style={{ borderBottom: '1px solid rgba(209,111,84,0.1)' }}>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 flex items-center justify-center border"
              style={{ background: 'transparent', borderColor: 'rgba(209,111,84,0.2)', borderRadius: 'var(--radius-sm)' }}>
              <Filter size={14} style={{ color: '#333333' }} />
            </div>
            <div>
              <p className="text-[13px] uppercase tracking-[1.4px] font-tags" style={{ color: '#333333' }}>Filters</p>
              <p className="text-[10px] uppercase tracking-[1px] mt-0.5 font-tags" style={{ color: hasCases ? '#333333' : '#888888' }}>
                {hasCases ? `OPTIONS FROM ${cases.length} CASES` : 'PRE-QUERY · ALL OPTIONS'}
                {activeCount > 0 && ` · ${activeCount} ACTIVE`}
              </p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 transition-colors" style={{ color: '#888888' }}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6 no-scrollbar">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              {local.outcomes.length > 0 && (
                <button onClick={() => setLocal(p => ({ ...p, outcomes: [] }))}
                  className="text-[10px] uppercase tracking-[1px] transition-colors font-tags" style={{ color: '#888888' }}
                  onMouseEnter={e => (e.currentTarget as HTMLElement).style.color = '#333333'}
                  onMouseLeave={e => (e.currentTarget as HTMLElement).style.color = '#888888'}>
                  Clear
                </button>
                ...
                return (
                  <button key={o.key} onClick={() => toggleOutcome(o.key)}
                    ...
                    style={{
                      ...
                      border: `1px solid ${on ? 'rgba(209,111,84,0.2)' : 'transparent'}`,
                      color: on ? '#333333' : '#888888',
                      borderRadius: 'var(--radius-sm)'
                    }}>
                    {on ? <Check size={12} /> : <span className="w-3 h-3 border" style={{ borderColor: 'rgba(209,111,84,0.2)', borderRadius: 'var(--radius-sm)' }} />}
                    {o.label}
                  </button>
                );
              })}
            ...
            {local.outcomes.length === 0 && (
              <p className="text-[10px] uppercase tracking-[1px] mt-2 font-tags" style={{ color: '#888888' }}>NONE SELECTED = ALL OUTCOMES</p>
            )}
          </div>
          ...
          {/* ── Active summary ── */}
          {activeCount > 0 && (
            <div className="p-4" style={{ background: 'rgba(128,128,128,0.02)', border: '1px solid rgba(209,111,84,0.2)', borderRadius: 'var(--radius-md)' }}>
              <p className="text-[10px] font-bold uppercase tracking-[1px] mb-3 font-tags" style={{ color: '#888888' }}>Active filters</p>
              <div className="flex flex-wrap gap-2">
        ...
        {/* Footer */}
        <div className="px-6 py-5 flex gap-3 shrink-0" style={{ borderTop: '1px solid rgba(209,111,84,0.1)' }}>
          <button onClick={() => setLocal({ ...DEFAULT_FILTERS, yearRange: [dynMinYear, dynMaxYear] })}
            ...
            style={{ color: '#888888', border: '1px solid rgba(209,111,84,0.2)', borderRadius: 'var(--radius-sm)' }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#888888'; (e.currentTarget as HTMLElement).style.color = '#333333'; }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(209,111,84,0.2)'; (e.currentTarget as HTMLElement).style.color = '#888888'; }}>
            Clear
          </button>
          <button onClick={() => { setFilters(local); onClose(); }}
            ...
            style={{ borderRadius: 'var(--radius-md)', background: '#D16F54', color: '#FFFFFF', boxShadow: 'none' }}>
            Apply Filters
          </button>
        </div>
      </div>
